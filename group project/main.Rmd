...
Tuning depth gives a test set RMSE of 35.64 and depth equal to just one layer.
...
Tuning alpha gives a value of 0.001 and a test RMSE score of 35.64, which is the same as with just the depth.
...
A random forest with trees set to 100 gives a test RMSE of 135.63, which is a lot worse than the decision tree, but still better than lasso, ridge, and basic linear regression. Cross validation was skipped due to extremely long computation times, but manually choosing 100 trees seemed to give approximately the best performance.
...
Tuning depth and lambda simultaneously for xgboost gives a depth of 2 and lambda equal to 0.01. The test RMSE is 173.16, making it the worst of the tree based methods.
