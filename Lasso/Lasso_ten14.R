...
for (i in seq(length(components_seq))) {
  ...
  lambda <- 10^seq(lambda_min, lambda_max, length.out = 10000)
  ...
  if (resolve_bool) {
    lambda <- 10^seq(temp_lambdaMin, temp_lambdaMax, length.out = 10000)
    cv_lasso <- cv.glmnet(X_train, Y_train, alpha = 1, lambda = lambda)
  }
...
lassoTen14_BestModel <- best_model_list[which.min(cv_mse)]
saveRDS(lassoTen14_BestModel, "Lasso/BestModels/BestLassoTen14Model.rds")

write.csv(lassoPCA_Results,"Lasso/Results/Ten14.csv", row.names = FALSE)
